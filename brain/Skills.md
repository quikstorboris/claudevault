...
- `qmd --index <name> query "..."` — hybrid BM25 + vector + LLM reranking (best quality, but see the cost warning below)
- `qmd --index <name> query "..." --no-rerank` — same hybrid retrieval, LLM reranking skipped. **On this machine, use this by default from the CLI.**
...
> [!warning] Reranking costs ~2s per candidate on this hardware (no discrete GPU).
> A plain `query` on a **novel** question measured 110s, of which 75s was
> reranking 22 chunks and 28s was HyDE expansion; a repeat of the same query
> returned in 7s purely from qmd's caches. Prefer `--no-rerank` interactively.
> `om`'s `search` tool already passes `rerank: false` for this reason — full
> diagnosis and numbers in
