...
**A third consumer, 2026-09-11**: the new Onboarding Work tab's facility tool-run history reused `useInfiniteLogFeed` directly, no changes needed — confirms it was actually written generic, not just generic-in-name. See [[Session 2026-09-11 — Onboarding Work Tab (Durable Tool-Run History)]].

## UnitPrep: review a previous session's uncommitted work like a stranger's PR before shipping it

Twice in one week (2026-09-09 cleanup after a stale-checkout incident, 2026-09-11 sorting a "developer role"/"tool runs" batch into coherent commits), Boris handed over substantial uncommitted work from a *different* session with instructions to review and push it. Both times, treating it as trusted-because-it-looks-thorough and just organizing it into commits would have shipped real bugs silently:

- A whole handler module (`api::tool_runs`) had **zero tests** — every other handler module in the codebase has at least permission/reaches-the-database smoke tests, so this was a real, catchable inconsistency, not a stylistic nitpick.
- Two RLS-writing functions ran their UPDATE against the raw connection pool with no GUCs set, so `rows_affected()` was silently always 0 — the module's own doc comment already documented this as a real live incident found *during that same uncommitted session*, but the regression tests proving the fix were marked `#[ignore]` and had never actually been run against the real schema until this review pass did it.

**The technique that catches this**: don't just skim diffs for "does this look reasonable" — actually run the full test/lint/typecheck suite, and for anything touching RLS or a live external system, run the `#[ignore]`d/live-only tests against the real (dev) database rather than trusting that they'd pass. Compare test coverage against sibling modules to catch a module with none. Query the live migration-tracking table (`_sqlx_migrations`) to confirm a migration was actually applied, not just written. This is slower than a trust-and-ship pass, but both real bugs above were only findable this way — a green default `cargo test` run doesn't touch `#[ignore]`d tests, and nothing short of reading the actual handler file surfaces "this module has no tests at all."

**Applied at scale again 2026-09-09**, prompted by an external (Grok) codebase review — but every claim was independently re-verified against the actual code before acting, catching two overstatements (see [[Session 2026-09-09 — Codebase Audit Follow-Through, God-File Refactors, README Rewrite, and Coherent Ship]] for the full correction). Split: `unitprep-ui`'s facility detail page (2283→213 lines, 9 tab components under `components/facility/`), `unitprep-api`'s `clients_facility_policies_edit.rs` (965→5 files, one per policy category) and `clients/sync.rs` (1422→4 files: progress/refresh/orchestrator). Also consolidated 14 files' worth of duplicated `bad_request`/`not_found`/`conflict` response-builders into 3 shared functions — a DRY-not-size finding, but the same "genuinely separable concept, name it, extract it" judgment call this pattern is about. Two other candidates were reviewed and correctly left alone: `repository.rs` (1404 lines, but ~50% test code, ~707 lines of cohesive logic) and `auth_register.rs` (single WebAuthn ceremony) — both would have been actively hurt by splitting on line-count alone.

**A miss, 2026-09-23/24**: `unitprep-api`'s `router.rs` grew from 842 to 1909 lines in one session (the `GatedRouter`/`RouteAccess` permission-manifest work — see [[Key Decisions]]) without being flagged in the moment, despite the 2026-08-14 standing instruction below to surface exactly this kind of growth unprompted. It only surfaced when a separately-requested codebase audit caught it afterward, and only because that audit's own file-size table happened to have been generated *before* the growth and so read as stale on comparison — not because the rule fired proactively while the file was actively growing. Root cause: the rule was recalled and applied for the *substance* of that session's work (permission gating, a real security gap) but never re-triggered afterward against the resulting file's own size, even though 1909 lines comfortably clears every threshold this rule names. **Fix applied**: added an explicit, standing "check file size after finishing, not just before starting" instruction directly to both `unitprep-api/CLAUDE.md` and `unitprep-ui/CLAUDE.md` (previously this rule lived only here in the vault, reached via `recall`/`search` on non-trivial *design* decisions — a routine multi-file edit that grows one file substantially doesn't reliably trigger that check, matching the vault's own documented lesson in its `CLAUDE.md` about prohibitions living in the vault alone getting skipped). `router.rs` itself was split in the same pass this was caught — see [[Key Decisions]].
