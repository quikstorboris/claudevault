...
- [[Process Street Integration — Kickoff & Findings|OO — Process Street Integration]] — new effort 2026-08-28: source OO's client/facility records from Process Street instead of manual entry. **Phases 0-2 fully shipped** ([[Phase 0-2 — Foundations, Ingestion Pipeline & Search]]), **Phase 3 ("Add to OO" confirmation screen), Phase 5 (re-sync + new Activity Logs feature), and Phase 4 items 1-3 (read-only Company page, facility rail, General + Facility Policies tabs) all shipped 2026-09-02**, plus three real bugs found and fixed from live usage on 2026-09-03 (Elavon data never being written, a page-blink/performance issue, a Dropbox link redesigned into a button) — full detail in [[Session 2026-09-02–03 — Confirmation Screen, Re-sync, Activity Logs & Client Record UI]]. See [[Implementation Plan]] for the full phase-by-phase build log and [[Client & Facility Schema (Process Street-Sourced)]] for schema decisions; all kept current, read those rather than this summary for anything specific. **Current state, 2026-09-03**: substantial uncommitted work in both repos' working trees (`unitprep-api` last commit `1bce4c8`/`v1.8.21`; `unitprep-ui` last commit `56ec716`/`v1.5.10`) — everything above is built, tested, and verified (backend `cargo test`/`clippy` clean, frontend `tsc`/`lint`/`vitest`/`build` clean, release binary rebuilt) but **not yet committed or pushed**; commit only when Boris asks. **Next**: Phase 4 items 4-7 (Users tab, Elavon tab, DropBox connection flow, `ps_task_status` indicators), person-name search (Phase 2's other half), Contract Order (lowest priority, deferred), and the encrypted-at-rest PS API key storage (agreed design, not started). A full auth/RLS backdoor audit came back clean early on — see [[Auth & RLS Security Audit — 2026-08-28]].
