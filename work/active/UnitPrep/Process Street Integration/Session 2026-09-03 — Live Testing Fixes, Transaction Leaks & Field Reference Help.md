...
Everything in this note plus the prior session's work: `unitprep-api` `v1.9.0` (4 commits, `1bce4c8..c95e1c3` — confirmation flow/correlation, re-sync+Activity Logs+archiving, Phase 4 UI+Elavon tab+live-usage fixes, version bump), `unitprep-ui` `v1.6.0` (4 commits, `56ec716..33d2fd4` — same grouping). Both `origin/main` clean, both tags pushed. Commit boundaries are thematic, not perfectly atomic -- a handful of shared files (`router.rs`/`mod.rs`, `merchant_account_mapping.rs`, `create.rs`) carry a few days' worth of intermingled changes each, landed in whichever commit best represents their dominant content, since retroactively splitting by hunk across a multi-day, single working tree wasn't worth the time/risk against a solo-repo history.
